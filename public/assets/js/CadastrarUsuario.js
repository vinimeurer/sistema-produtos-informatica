...
function validarDocumento(input) {
    ...
    if (!validaCPF(cpf)) {
        ...
        errorMessage.style.display = 'none'; 
        input.style.borderColor = ''; 
        input.style.color = ''; // Reseta a cor do texto
        input.style.backgroundColor = ''; // Reseta a cor de fundo
    }
}
...
function removerMascara() {
    var documentoField = document.getElementById('documento');
    var cpf = documentoField.value.replace(/\D/g, ''); // Remove a máscara antes de enviar

    if (!validaCPF(cpf)) {
        alert("O CPF inserido é inválido!"); // Alerta se o CPF for inválido
        return false; // Evita o envio do formulário se o CPF for inválido
    }

    documentoField.value = cpf; // Atualiza o valor
}
...
function validarFormulario() {
    ...
    var cpfValido = validaCPF(documento.replace(/\D/g, ''));
    
    var senhaValida = validarSenha(senha);
    document.getElementById('cep').addEventListener('input', validarFormulario);
    ...
    if (nome && documento && senhaValida && login && cpfValido) {
        ...
        submitBtn.style.color = "#ffffff"; //"#6c757d";
...
document.getElementById('login').addEventListener('input', validarFormulario);
